...
out_dir="out02_adni_fmri_t1_dpsfa"
...
do
    ...
    ls -d $data_dir_fmri/$sub/*rs[f,F]MRI*/*/I*/*.nii>temp_fun_sub.txt
    ls -d $data_dir_fmri/$sub/*fcMRI*/*/I*/*.nii>>temp_fun_sub.txt
    ls -d $data_dir_fmri/$sub/*Resting_State_fMRI/*/I*/*.nii>>temp_fun_sub.txt
    ls -d $data_dir_fmri/$sub/AXIAL_RS_fMRI*/*/I*/*.nii>>temp_fun_sub.txt
    echo 'temp_fun_sub.txt:'
    cat temp_fun_sub.txt

    t1_file=$(ls -d $data_dir_t1/$sub/MPRAGE/*/S*/*.nii | tail -1)
    t1_session=$(echo $t1_file | awk -F '/' '{print $8}')
    echo $t1_file
    ...
    do
        ...
        mkdir -p $out_dir/FunImg/${subid}-${sesid}/
        mkdir -p $out_dir/T1Img/${subid}-${sesid}

        cp -l $line2 $out_dir/FunImg/${subid}-${sesid}/rest_bold.nii
        ...
        cp -l $t1_file $out_dir/T1Img/${subid}-${sesid}/T1w.nii
